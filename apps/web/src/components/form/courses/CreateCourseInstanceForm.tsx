...
import {
	...
	type CoursesInfo,
	CreateCourseInstanceSchema, ON_SUBMIT_INVALID_MSG
...
import RadioInput, {type RadioInputProps} from "@/components/form/general/RadioInput.tsx"
...
import NumberInput, {type NumberInputProps} from "@/components/form/general/NumberInput.tsx";
...
export default function CreateCourseInstanceForm() {
	...
	const navigate = useNavigate({ from: "/home/current-period/course-instance/" })
	...
	const breakdownArray = useSelector(form.store, (state => state.values.breakdown))

	const totalPercentage = breakdownArray.reduce((previousVal, currentVal) => previousVal + currentVal.percentage, 0);
	const laboratoryDetailBreakdownArray = breakdownArray.find(breakdown => breakdown.type === "NESTED")?.laboratoryDetails?.breakdown
	const labDetailTotalPercentage = laboratoryDetailBreakdownArray?.reduce((previousVal, currentVal) => previousVal + currentVal.percentage, 0) ?? -1
	console.log(labDetailTotalPercentage)
	...
	return (
		...
		>
			...
			/>
			{ breakdownArray.length > 0 && <h2 className={"text-xl mt-4"}>Evaluación del curso</h2> }
			<Field
				name={"breakdown"}
				mode={"array"}
				>
						...
						return (
							<div className={"flex flex-col gap-8 border border-red-400"}>
								{fieldApi.state.value.map((breakdown, i) => {
									return (
										<div key={`breakdown[${i}]-ctn`} className={"border border-primary-300"}>
											<div className={"relative"}>
												...
												>
													{(nameField) => {
														...
														const {name, handleBlur, handleChange, state} = nameField
														const inputProps: InputProps<string> = {
															name: `${name}-${i}-name`,
															id: `${name}-${i}-name`,
															value: state.value,
															type: "text",
															syncValueToState: (e) => handleChange(e.currentTarget.value),
															isBlurred: isBlurred || submissionAttempts > 0,
												...
												<Trash2
													className={"text-red-700 cursor-pointer hover:scale-110 transition-all duration-300 ease-in-out absolute top-4 left-110"}
													onClick={() => {
														buildModal({
															...
															modalContent: "Eliminarás completamente la evaluación. No podrás recuperar el progreso que tenías"
														})
														}
													}
												/>
											...
											>
												{(percentageField) => {
													...
													const {name, handleBlur, handleChange, state} = percentageField
													const numberInputProps : NumberInputProps = {
														name: `${name}-${i}-percentage`,
														id: `${name}-${i}-percentage`,
														value: state.value.toString(),
														syncValueToState: (e) => handleChange(Number(e.currentTarget.value)),
														isBlurred: isBlurred || submissionAttempts > 0,
													...
													return (
														<NumberInput {...numberInputProps} key={`${name}-${i}-percentage`}/>
													)
												}}
											</Field>
											<Field
												...
												{(typeField) => {
													...
													const generalRadioProps : Omit<RadioInputProps, "value" | "labelText" | "radioId"> = {
														handleBlur,
														syncValueToState: (e) => handleChange(e.target.value as BreakdownCategory),
														radioGroupName: `breakdown[${i}].type`,
													}
													return (
														<div className={"flex flex-col gap-2"}>
															<h2 className={"text-xl"}>La evaluación tiene</h2>
															<RadioInput value={"NESTED"} labelText={"Parte de teoría y laboratorio"} radioId={`breakdown[${i}].nested`} {...generalRadioProps} key={`breakdown[${i}].nested`}/>
															<RadioInput value={"NOT-NESTED"} labelText={"Tiene subdivisiones"} radioId={`breakdown[${i}].not-nested`} {...generalRadioProps} key={`breakdown[${i}].not-nested`}/>
															<RadioInput value={"STANDALONE"} labelText={"Es un solo porcentaje"} radioId={`breakdown[${i}].standalone`} {...generalRadioProps} key={`breakdown[${i}].standalone`}/>
															{ breakdown.type === "NESTED" && (
																<>
																	<h2 className={"text-xl mt-4"}>Evaluación del curso de laboratorio</h2>
																	<Field
																		...
																		{(labDetailBreakdown) => {
																			return (
																				<div className={"flex flex-col gap-8"}>
																					{labDetailBreakdown.state.value?.map((_, labIndex) => {
																						return (
																							<div
																								className={"relative"}
																							...
																							>
																								...
																								<Field name={`breakdown[${i}].laboratoryDetails.breakdown[${labIndex}].percentage`}>
																									{(labDetailBreakdownPercentField) => {
																										const {errors, isBlurred} = labDetailBreakdownPercentField.state.meta
																										const {name, handleBlur, handleChange, state} = labDetailBreakdownPercentField
																										const numberInputProps : NumberInputProps = {
																											name: `${name}-${i}-percentage`,
																											id: `${name}-${i}-percentage`,
																											...
																											syncValueToState: (e) => handleChange(Number(e.currentTarget.value)),
																											isBlurred: isBlurred || submissionAttempts > 0,
																											label: "¿Cuál es el porcentaje de la evaluación de laboratorio?",
																											error: errors[0]?.message,
																											color: "#F9FCFC",
																											handleBlur,
																										}
																								...
																								>
																									{(labDetailTypeField) => {
																										const {handleBlur, handleChange} = labDetailTypeField
																										const generalRadioProps : Omit<RadioInputProps, "value" | "labelText" | "radioId"> = {
																											handleBlur,
																											syncValueToState: (e) => handleChange(e.target.value as BreakdownCategory),
																										...
																										return (
																											<div className={"flex flex-col gap-2"}>
																												<h2 className={"text-xl"}>La evaluación tiene</h2>
																												<RadioInput value={"NOT-NESTED"} labelText={"Tiene subdivisiones"} radioId={`breakdown[${i}].laboratoryDetails.breakdown[${labIndex}].not-nested`} {...generalRadioProps} key={`breakdown[${i}].laboratoryDetails.breakdown[${labIndex}].not-nested`}/>
																												<RadioInput value={"STANDALONE"} labelText={"Es un solo porcentaje"} radioId={`breakdown[${i}].laboratoryDetails.breakdown[${labIndex}].standalone`} {...generalRadioProps} key={`breakdown[${i}].laboratoryDetails.breakdown[${labIndex}].standalone`}/>
																											</div>
																											)}}
																										</Field>
																									</div>
																					...
																					{labDetailTotalPercentage < 100 && (
																						<AddItem title={"Agregar evaluación para el laboratorio"} action={() => labDetailBreakdown.pushValue({ name: "", percentage: 0,
																							type: "STANDALONE" })}
																						/>
																					)}
								...
								{ totalPercentage < 100 && <AddItem title={"Agregar Evaluación"} action={() => fieldApi.pushValue({
									name: "",
									percentage: 0,
									type: "STANDALONE"
								})} /> }
			...
			{totalPercentage === 100 && (
				<div className={"w-full flex items-center justify-center"}>
					<Button text={"Guardar materia"} type={"submit"} styleType={"primary"} isDisabled={false}/>
				</div>
			)}
		</form>
	)
